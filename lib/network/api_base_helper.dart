...
import 'package:townerapp/core/core.dart';
...
class ApiBaseHelper {
  ...
  Future<dynamic> put(String url, dynamic body) async {
    ...
    _log.logPrint('Api Put, data $body');
    var responseJson;
    try {
      dio.interceptors.clear();
      dio.interceptors.add(LogInterceptor(responseBody: false));
  ...
  Future<dynamic> delete(String url, {dynamic header, dynamic body, dynamic param}) async {
    _log.logPrint('Api delete, url $url');
    var responseJson;
    try {
      dio.interceptors.clear();
      dio.interceptors.add(LogInterceptor(responseBody: false));
      final response = await dio.delete(Uri.parse(url).toString(), data: body, queryParameters: param, options: options());
      responseJson = _returnResponse(response);
    } on DioException catch (e) {
      if (e.type == DioExceptionType.badResponse) {
        _log.logPrint('Error caught: ${e.response}');
